...
int seg(int thr, char *c1, char *c2, mtz *mat){ // Função para segmentar uma matriz
...
int set_mat(char *file, mtz *mat){ // Função para atribuir os dados de uma matriz em um arquivo txt ou imm
    FILE *fl;
    int num, aux, i, j, row, col;
...
int tam_file(char *file, int *l, int *c){ // Função para determinar a quantidade de linhas e colunas de um arquivo .txt ou .imm
    ...
    if (type_of(file) == TXT){ // Verifica se o arquivo é txt
        ...
        if(row == 0 || col == 0){return UNDEFINED_ERROR;}
        ...
        *c = col; // Atribui a contagem de colunas no retorno de colunas
        ...
        if(row <= 1 || col <= 1){return UNDEFINED_ERROR;}
        *l = row; // Atribui a contagem de linha no retorno de linhas
        *c = col; // Atribui a contagem de colunas no retorno de colunas
...
int set_arq(char *file, mtz **mat){ // Função para atribuir os dados de um arquivo em uma matriz
    FILE *fl;
    int aux,num,row,col;
    if (type_of(file) == TXT){ // Verifica se o arquivo é txt
        fl = fopen(file, "r"); // Abre o arquivo para leitura
